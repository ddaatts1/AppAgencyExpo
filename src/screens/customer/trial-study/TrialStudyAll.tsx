...
import formatDate, { dateCompareWithCurrentDate } from "../../../components/formatDate";
...
const TrialStudyAll = () => {
    ...
    const packageTitleMonth = (product: any, expiredDate: any) => (
        <View style={{flexDirection: 'column'}}>
            {product?.map((p: any, index: any) => {
                if (dateCompareWithCurrentDate(expiredDate)) {
    ...
    trialsData?.data?.map((t:any)=>{
        const rowData = [];
        rowData.push([fullname_phone(t.fullname,t.phone_number), formatDate(t.actived_date), formatDate(t.expired_date), packageTitleMonth(t.product, t.expired_date), t.number_study?t.number_study:'0', button(t.email)]);
