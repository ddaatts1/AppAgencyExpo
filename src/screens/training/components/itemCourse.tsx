...
const { formatD, } = useCommon();

const ItemCourse = ({ navigation, dataCourses }: any) => {
    ...
    return (
        dataCourses?.map((item: any) => {
            ...
            return item?.courses?.map((data: any, index: any) => {
                ...
                return (
                    <ItemChage key={index} color={"#0288D1"} >
                        ...
                        <View >
                            ...
                            <Text numberOfLines={3} style={styles.title}>{data.name} </Text>
                            <Text style={{ color: "#0288D1", fontSize: 16, paddingHorizontal: 12, paddingBottom: 12 }} numberOfLines={2}>{data?.name_teacher}</Text>
                            <View style={{ flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 12, paddingBottom: 12 }}>
                                <View style={{ flexDirection: "row", alignItems: "center" }}>
                                    <SVG.Icon_Peoples />
                                    <Text style={{ fontSize: 14, color: "#0288D1" }}> {data?.studied}</Text>
                                </View>

                                {data?.charge == 1 ? <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center', borderRadius: 16, backgroundColor: "#0288D1", height: 36, }}
                                ...
                                </TouchableOpacity> : data?.charge == 0 && <View >
                                    <Text style={{ color: "#525252", fontSize: 14, textAlign: 'right' }}>{formatD(data?.fee)}</Text>
                                    <Text style={{ color: "#D51E03", fontSize: 18, fontWeight: 'bold' }}>{formatD(data?.fee_sale)}</Text>
