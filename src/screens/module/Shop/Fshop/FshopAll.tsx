...
const FshopAll = ({route,onAddToCartSuccess}: any) => {
    ...
    useEffect(()=>{
        const fetchData = async () => {
           ...
           await fetchListCardInfo(data).then((result)=>setDataToShow(result))
        }
        fetchData()
    },[id,slug])
    // useFocusEffect(
   ...
   async function handleAddToCart() {
        const item = {
            ...
            color: selectedColor,
            size: selectedSize,
            category: ProductType.Fshop,
            service: service
        }
        StorageHelper.addToCart(item)
        setAddToCart(false)
        Toast.showWithGravityAndOffset('Đã thêm vào giỏ hàng', Toast.LONG, Toast.BOTTOM, 25, 50, {backgroundColor: '#fff'});
