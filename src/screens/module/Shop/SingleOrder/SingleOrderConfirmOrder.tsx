...
import generateSignedQueryString, {
    generateSignedFshop,
    generateSignedSingle,
} from '../../../card-warehouse/change-card/generateSignedQueryString';
...
const SingleOrderConfirmOrder = ({route}: any) => {
    const {item, quantity, color, size} = route?.params;

    const navigation = useNavigation<StackNavigationProp<ParamListBase>>();
    const {futureLang} = useFutureLang();
    const {createdOrder,createNewOrder,isCreatingOrder,orderCheck} = useOrder();
    ...
    async function handleOrder() {
        ...
        const totalPrice = (item.price_discount || item.value) * quantity;
        const orderDetail = {
            order_detail: [{
                id: item.id,
                quantity: quantity,
        ...
        orderSingle(orderDetail);


    }

    async function orderSingle(orderDetail:any) {
        try {
    ...
    return (
        <View style={styles.container}>
                ...
                <View style={styles.box}>
                                    ...
                                    <View style={{width: '80%'}}>
                                        ...
                                        {item.price_discount !== undefined && item.price_discount !== 0 && (
                                            <View style={styles.priceDiscount}>
                                                ...
                                                <Text style={{textDecorationLine: 'line-through', textDecorationStyle: 'solid' }}>{formatPriceVNDShop(item?.value)}</Text>
                                            </View>
                                        )}
                    ...
                    <View style={{width: '96%', flexDirection: 'row', marginBottom: 4, marginTop: 24}}>
                        ...
                        <View style={styles.price}>
                            <SVG.VND/>
                            <Text>{formatPriceVND(quantity * (item?.price_discount || item?.value))}</Text>
                        </View>
                    </View>
                    ...
                    <ModalPoup visible={addressNotFound}>
                        ...
                        <View style={styles.confirmBtn}>
                            <View style={{width: '100%', flexDirection: 'row'}}>
                                <TouchableOpacity
                                    onPress={() => setAddressNotFound(false)}
                                    style={[{backgroundColor: '#fff'}, styles.loginBtn]}>
            ...
            <View style={styles.footer}>
                <View style={{}}>
                    <Text style={{fontSize: 16, fontFamily: 'Roboto', fontStyle: 'normal', fontWeight: '400'}}>
                ...
                <TouchableOpacity onPress={() => handleOrder()} style={styles.button}>
                    {isCreatingOrder ? <Loading/> : <Text style={styles.confirm}>Đặt hàng</Text>}
...
export default SingleOrderConfirmOrder;
